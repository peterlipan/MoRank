...
class KneeOsteoarthritisData:
    ...
    def _read_folder(self, path):
        ...
        for grade in grades:
            ...
            files = [os.path.join(grade_path, f) for f in os.listdir(grade_path) if f.endswith('.png')]
            for file in files:
    ...
    def get_official_train_test(self):
        return (
            KneeOsteoarthritisDataset(self.train_df, self.n_classes, self.n_features, transform=self.train_transform),
            KneeOsteoarthritisDataset(self.test_df, self.n_classes, self.n_features, transform=self.test_transform)
        )
